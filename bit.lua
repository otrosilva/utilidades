...
-- Metatabla Bitacora
-- funciones:
-- - new(ruta,editor,show)
-- - listar()
-- - mostrar(bitacora)
-- - editar(bitacora)
-- - borrar(bitacora,todo=false)
-- - config()
...
function Bitacora:editar(bitacora)
    ...
    -- Construir el comando para editar el archivo
    local command = self.editor .. " " .. rutaCompleta

    -- Ejecutar el comando usando os.execute
...
--- Agrega una nueva entrada a bitácora.
function Bitacora:agregar(bitacora, texto)
    local rutaCompleta = self.ruta .. bitacora .. ".txt"

    -- Abrir el archivo en modo "append" (agregar al final)
    local archivo, errorMensaje = io.open(rutaCompleta, "a")
    if not archivo then
        print("Error al abrir el archivo: " .. errorMensaje)
        return
    end

    -- Escribir el texto en una nueva línea con marca de tiempo
    local prefijo = os.date("%Y-%m-%d %H:%M:%S") .. " [" .. bitacora .. "]"
    local log_entry = string.format("%s: %s", prefijo, texto) -- Formato simplificado
...
--- Borra la última línea o todo el archivo de bitácora, con o sin confirmación.
-- @param todo (opcional) Si es true, borra todo el archivo pidiendo confirmación. Si es false, borra solo la última línea sin pedir confirmación. Por defecto es false.
...
-- return Bitacora
-- local bit = Bitacora.new("~/iCloud/Docs/Registros/", "hx +9999", "tail -5")
-- bit:listar()
-- bit:mostrar("prueba")
-- bit:editar("prueba")
-- bit:agregar("prueba", "texto de prueba 1")
-- bit:agregar("prueba", "texto de prueba 2")
-- bit:editar("prueba")
-- bit:config()
--

-- Inicio de ejecución
-- local Bitacora = require("Bitacora")
local Bit = Bitacora.new("~/iCloud/Docs/Registros/", "hx +9999", "cat")
